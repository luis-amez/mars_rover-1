...
var orders = ['f','f','r','f','f','l','f','l','f','b','l','f','f' ];
...
// Use an if to check if the Rover is close to crash out of the grid 

      if(Myrover.position.x == 5 || Myrover.position.y == 5 || Myrover.position.x == -5 || Myrover.position.y == -5){
           console.log("The rover is close to crash out the grid");
  };
...
we will say "If the rover move, the rover will crash with an obstacle" and if is not the same position, will continue with the movement"*/
